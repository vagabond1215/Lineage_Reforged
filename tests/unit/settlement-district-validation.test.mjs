...
import { validateSettlementDistricts } from "../../tools/content-lint/settlement-districts.mjs";

const ROOT = process.cwd();
const SETTLEMENT_DISTRICT_PATH = "packages/content/base/world/settlement_districts.json";
const SCHEMA_PATH = "packages/schemas/world/settlement-district.schema.json";
const VALIDATOR_PATH = "tools/content-lint/settlement-districts.mjs";
const TEST_PATH = "tests/unit/settlement-district-validation.test.mjs";
...
function settlementDistrict(overrides = {}) {
  return {
    id: "settlement_district.aurelis.market_quarter",
    slug: "market_quarter",
    name: "Market Quarter",
    aliases: [],
    summary: "In-memory settlement district fixture for static descriptive validation.",
    parentSettlementId: "settlement.aurelis",
    districtType: "market_quarter",
    functionalTags: ["trade"],
    placeRoleTags: ["civic_exchange"],
    status: "planned",
    sourceAuthorityNotes: [
      "Fixture only; no live settlement-district content is authored by this test."
    ],
    notes: [
      "Static district identity only; no coordinates, routes, services, inventories, ownership, Knowledge state, runtime, UI, storage, command, event, reward, or gameplay behavior."
    ],
    ...overrides
  };
}

function makeInput(records = [settlementDistrict()], overrides = {}) {
  return {
    relativePath: SETTLEMENT_DISTRICT_PATH,
...
function validateInput(input = makeInput()) {
  return validateSettlementDistricts(input);
}
...
test("accepts valid planned settlement district for an existing settlement", () => {
  assert.deepEqual(validateInput(), {
    ok: true,
    settlementDistrictIds: ["settlement_district.aurelis.market_quarter"]
  });
});
...
test("accepts allowed lifecycle statuses", async (t) => {
  for (const status of ["planned", "active", "retired"]) {
    await t.test(status, () => {
      assert.equal(validateInput(makeInput([settlementDistrict({ status })])).ok, true);
    });
  }
});

test("accepts all controlled district types", async (t) => {
  for (const districtType of schema.$defs.districtType.enum) {
    await t.test(districtType, () => {
      assert.equal(
        validateInput(makeInput([
          settlementDistrict({
            id: `settlement_district.aurelis.${districtType}`,
            slug: districtType,
            districtType
          })
        ])).ok,
...
test("allows duplicate district slugs under different parent settlements", () => {
  const records = [
    settlementDistrict(),
    settlementDistrict({
      id: "settlement_district.vinecross.market_quarter",
  ...
  assert.deepEqual(validateInput(makeInput(records)), {
    ok: true,
    settlementDistrictIds: [
      "settlement_district.aurelis.market_quarter",
      "settlement_district.vinecross.market_quarter"
    ]
  });
});

test("does not mutate settlement-district validation inputs", () => {
  const input = makeInput();
...
test("rejects malformed wrapper and strict record failures", async (t) => {
  const cases = [
    ...
    [
      ...
      (input) => { record(input).settlementDistrictIds = ["settlement_district.aurelis.market_quarter"]; },
      /unsupported property 'settlementDistrictIds'/
    ]
  ];
...
test("rejects settlement-district identity and parent failures", async (t) => {
  const cases = [
    [
      "malformed id",
      (input) => { record(input).id = "district.aurelis.market_quarter"; },
      /id must match pattern \^settlement_district/
    ],
    [
      "id slug mismatch",
      (input) => { record(input).slug = "harbor_ward"; },
      /slug must match final id segment 'market_quarter'/
    ],
    [
    ...
    [
      ...
      (input) => {
        record(input).id = "settlement_district.missing.market_quarter";
    ...
    [
      ...
      /duplicate settlement-district id 'settlement_district\.aurelis\.market_quarter'/
    ],
    [
      "duplicate slug within one parent settlement",
      (input) => {
        const duplicate = structuredClone(record(input));
        duplicate.id = "settlement_district.aurelis.market_quarter_duplicate";
        input.wrapper.records.push(duplicate);
      },
      /duplicate settlement-district slug 'market_quarter' under parent 'settlement\.aurelis'/
    ]
  ];
...
test("rejects duplicate arrays, invalid vocabularies, and malformed tags", async (t) => {
  const cases = [
    ["duplicate aliases", (input) => { record(input).aliases = ["Market Quarter", "Market Quarter"]; }, /aliases must contain unique items/],
    ["duplicate functional tags", (input) => { record(input).functionalTags = ["trade", "trade"]; }, /functionalTags must contain unique items/],
    ["duplicate place role tags", (input) => { record(input).placeRoleTags = ["civic_exchange", "civic_exchange"]; }, /placeRoleTags must contain unique items/],
    ...
    ["invalid district type", (input) => { record(input).districtType = "service_runtime"; }, /districtType must be one of the schema enum values/],
    ["invalid lifecycle status", (input) => { record(input).status = "draft"; }, /status must be one of the schema enum values/],
    ["non-lower-snake functional tag", (input) => { record(input).functionalTags = ["Market"]; }, /functionalTags\[0\] must match pattern/],
    ["non-lower-snake place role tag", (input) => { record(input).placeRoleTags = ["civic exchange"]; }, /placeRoleTags\[0\] must match pattern/],
...
test("schema, validator, and focused test exist while live content and normal lint registration remain absent", async () => {
  ...
  assert.match(schemaTestSource, /packages\/schemas\/world\/settlement-district\.schema\.json/);
  assert.doesNotMatch(contentLintSource, /settlement_districts\.json/);
  assert.doesNotMatch(contentLintSource, /settlement-districts\.mjs/);
});
