const BUSINESS_ID_PATTERN = /^business\.[a-z0-9]+(?:_[a-z0-9]+)*$/;
...
const FORMS = new Set(["company", "partnership", "cooperative", "other", "unknown"]);
...
function validateRecord(record, index, issues) {
  ...
  if (Object.hasOwn(record, "id")) {
    if (typeof record.id !== "string" || !BUSINESS_ID_PATTERN.test(record.id)) {
      issues.push(`${recordPath}.id must match ^business\\.[a-z0-9]+(?:_[a-z0-9]+)*$`);
    }
  }
  ...
  if (
    ...
    BUSINESS_ID_PATTERN.test(record.id) &&
    SLUG_PATTERN.test(record.slug) &&
    record.id !== `business.${record.slug}`
  ) {
    issues.push(`${recordPath}.id must equal business.${record.slug}`);
  }
  ...
  if (Object.hasOwn(record, "form") && !FORMS.has(record.form)) {
    issues.push(`${recordPath}.form must be one of company, partnership, cooperative, other, unknown`);
  }
...
export function validateBusinesses(wrapper, options = {}) {
  const relativePath = options.relativePath ?? "packages/content/base/civilization/businesses.json";
  const issues = [];
