...
const MAP_FEATURE_ID_PATTERN = /^map_feature\.([a-z0-9]+(?:_[a-z0-9]+)*)$/;
...
const WORLD_MAP_FEATURE_ID_PATTERN = /^world_map_feature\.[a-z0-9]+(?:_[a-z0-9]+)*$/;
const VISUAL_FEATURE_ID_PATTERN = /^feature\.[a-z0-9]+(?:[._][a-z0-9]+)*$/;
const VISUAL_ALIAS_PATTERN = /^(?:world_map_feature|feature)\./;

const ALLOWED_VISUAL_GROUPS = {
  coastlines: "coastline",
  riverFeatures: "river",
  mountainFeatures: "mountain",
  passFeatures: "pass"
};

const DISALLOWED_VISUAL_GROUPS = {
  regionFootprints: "region footprint",
  crossingFeatures: "crossing",
  climateZones: "climate zone",
  biomeZones: "biome zone"
};
...
function buildVisualFeatureIndex(worldMapFeatures) {
  if (!Array.isArray(worldMapFeatures)) {
    throw new Error("world.world_map_features records must be an array");
  }

  const aggregatesById = new Map();
  worldMapFeatures.forEach((record, recordIndex) => {
    if (!isObject(record) || typeof record.id !== "string" || !WORLD_MAP_FEATURE_ID_PATTERN.test(record.id)) {
      throw new Error(`world.world_map_features records[${recordIndex}] must provide a canonical aggregate id`);
    }
    if (aggregatesById.has(record.id)) {
      throw new Error(`world.world_map_features has duplicate aggregate id '${record.id}'`);
    }

    const allowedFeaturesById = new Map();
    const disallowedFeaturesById = new Map();

    for (const [groupName, featureKind] of Object.entries(ALLOWED_VISUAL_GROUPS)) {
      for (const feature of record[groupName] ?? []) {
        if (isObject(feature) && typeof feature.id === "string" && VISUAL_FEATURE_ID_PATTERN.test(feature.id)) {
          allowedFeaturesById.set(feature.id, featureKind);
        }
      }
    }

    for (const [groupName, featureKind] of Object.entries(DISALLOWED_VISUAL_GROUPS)) {
      for (const feature of record[groupName] ?? []) {
        if (isObject(feature) && typeof feature.id === "string") {
          disallowedFeaturesById.set(feature.id, featureKind);
        }
      }
    }

    aggregatesById.set(record.id, {
      allowedFeaturesById,
      disallowedFeaturesById
    });
  });
...
function assertUniqueVisualReferences(record, relativePath) {
  const seen = new Set();
  for (const reference of record.visualReferences) {
    const key = `${reference.visualAggregateId}:${reference.visualFeatureId}:${reference.relationship}`;
    if (seen.has(key)) {
      throw new Error(`${relativePath} record ${record.id} has duplicate visualReference '${key}'`);
    }
    seen.add(key);
  }
}
...
function validateVisualReference(reference, record, relativePath, visualAggregatesById) {
  const aggregate = visualAggregatesById.get(reference.visualAggregateId);
  if (!aggregate) {
    throw new Error(`${relativePath} visualReference aggregate '${reference.visualAggregateId}' is missing from world.world_map_features on record ${record.id}`);
  }
  if (aggregate.allowedFeaturesById.has(reference.visualFeatureId)) {
    return;
  }
  const disallowedKind = aggregate.disallowedFeaturesById.get(reference.visualFeatureId);
  if (disallowedKind) {
    throw new Error(`${relativePath} visualReference '${reference.visualFeatureId}' targets disallowed ${disallowedKind} geometry on record ${record.id}`);
  }
  throw new Error(`${relativePath} visualReference feature '${reference.visualFeatureId}' is missing from aggregate '${reference.visualAggregateId}' on record ${record.id}`);
}

export function validateMapFeatures({
  ...
  assertUniqueField(records, "id", relativePath, "map-feature id");
  assertUniqueField(records, "slug", relativePath, "map-feature slug");
  ...
  const visualAggregatesById = buildVisualFeatureIndex(worldMapFeatures);

  for (const record of records) {
    assertNoForbiddenFields(record, relativePath);
    const match = MAP_FEATURE_ID_PATTERN.exec(record.id);
    if (!match || match[1] !== record.slug) {
      throw new Error(`${relativePath} record ${record.id} id must equal map_feature.${record.slug}`);
    }
    for (const alias of record.aliases) {
      if (VISUAL_ALIAS_PATTERN.test(alias)) {
        throw new Error(`${relativePath} record ${record.id} aliases must not bridge to visual id '${alias}'`);
      }
    }

    assertUniquePlaceAnchors(record, relativePath);
    let hasRegionOrLocalityAnchor = false;
    for (const anchor of record.placeAnchors) {
      if (anchor.placeType === "region" || anchor.placeType === "region_locality") {
        hasRegionOrLocalityAnchor = true;
      }
      validatePlaceAnchor(anchor, record, relativePath, indexes);
    }
    if (!hasRegionOrLocalityAnchor) {
      throw new Error(`${relativePath} record ${record.id} must include at least one region or region_locality placeAnchor`);
    }

    assertUniqueVisualReferences(record, relativePath);
    for (const reference of record.visualReferences) {
      validateVisualReference(reference, record, relativePath, visualAggregatesById);
    }
  }

  return {
    ok: true,
    mapFeatureIds: records.map((record) => record.id).sort()
  };
}
