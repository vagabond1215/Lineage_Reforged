const FACTION_ID_PATTERN = /^faction\.[a-z0-9]+(?:_[a-z0-9]+)*$/;
...
function validateRecord(record, index, issues) {
  ...
  if (Object.hasOwn(record, "id")) {
    if (typeof record.id !== "string" || !FACTION_ID_PATTERN.test(record.id)) {
      issues.push(`${recordPath}.id must match ^faction\\.[a-z0-9]+(?:_[a-z0-9]+)*$`);
    }
  }
  ...
  if (
    ...
    FACTION_ID_PATTERN.test(record.id) &&
    SLUG_PATTERN.test(record.slug) &&
    record.id !== `faction.${record.slug}`
  ) {
    issues.push(`${recordPath}.id must equal faction.${record.slug}`);
  }
  ...
  if (Object.hasOwn(record, "category") && !CATEGORIES.has(record.category)) {
    issues.push(`${recordPath}.category must be a supported faction category`);
  }
...
export function validateFactions(wrapper, options = {}) {
  const relativePath = options.relativePath ?? "packages/content/base/civilization/factions.json";
  const issues = [];
