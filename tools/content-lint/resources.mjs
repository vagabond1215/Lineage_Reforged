...
const RESOURCE_ID_PATTERN = /^resource\.([a-z0-9]+(?:_[a-z0-9]+)*)$/;
const COMMODITY_ID_PATTERN = /^commodity\.[a-z0-9]+(?:_[a-z0-9]+)*$/;
const LOWER_SNAKE_PATTERN = /^[a-z0-9]+(?:_[a-z0-9]+)*$/;
const PRODUCTION_STAGE_REF_PATTERN = /^(?:workplace|extract)\.[a-z0-9]+(?:[._][a-z0-9]+)*$/;
...
function buildProductionStageSet(productionChains) {
  const records = recordsFrom(productionChains, "civilization.production_chains");
  if (records === undefined) {
    return undefined;
  }
  const refs = new Set();
  const visit = (value) => {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (key === "stageRef" && typeof child === "string" && PRODUCTION_STAGE_REF_PATTERN.test(child)) {
        refs.add(child);
      }
      visit(child);
    }
...
function assertSafeTags(record, relativePath) {
  for (const tag of record.tags) {
    ...
    if (tagHasForbiddenFragment(tag)) {
      throw new Error(`${relativePath} record ${record.id} tag '${tag}' implies forbidden resource state or execution intent`);
    }
  }
}
...
function assertProductionStageRefs(record, productionStageRefs, relativePath) {
  for (const stageRef of record.relatedProductionStageRefs ?? []) {
    if (productionStageRefs === undefined) {
      throw new Error(`${relativePath} record ${record.id} relatedProductionStageRefs requires civilization.production_chains records`);
    }
    if (!productionStageRefs.has(stageRef)) {
      throw new Error(`${relativePath} record ${record.id} relatedProductionStageRefs '${stageRef}' is missing from civilization.production_chains stageRef values`);
    }
  }
}

export function validateResourcesContent({
  relativePath = "packages/content/base/world/resources.json",
  wrapper,
  schema,
  items,
  commodities,
  ...
  assertUniqueField(records, "id", relativePath, "resource id");
  assertUniqueField(records, "slug", relativePath, "resource slug");
  assertUniqueField(records, "name", relativePath, "resource name");

  const itemKeys = buildItemKeySet(items);
  const marketKeys = buildMarketKeySet(marketItemValues);
  const commodityIds = buildIdSet(commodities, "world.commodities");
  const floraIds = buildIdSet(flora, "world.flora");
  const faunaIds = buildIdSet(fauna, "world.fauna");
  const biomeIds = buildIdSet(biomes, "world.biomes");
  const habitatIds = buildIdSet(habitats, "world.habitats");
  const regionIds = buildIdSet(regions, "world.regions");
  const mapFeatureIds = buildIdSet(mapFeatures, "world.map_features");
  const productionStageRefs = buildProductionStageSet(productionChains);

  for (const record of records) {
    const idMatch = RESOURCE_ID_PATTERN.exec(record.id);
    if (!idMatch) {
      throw new Error(`${relativePath} record ${record.id ?? "<unknown>"} id must match resource.<slug>`);
    }
    ...
    assertIdRefs(record, "relatedCommodityIds", commodityIds, "world.commodities", relativePath);
    assertIdRefs(record, "relatedFloraIds", floraIds, "world.flora", relativePath);
    assertIdRefs(record, "relatedFaunaIds", faunaIds, "world.fauna", relativePath);
    assertIdRefs(record, "relatedBiomeIds", biomeIds, "world.biomes", relativePath);
    assertIdRefs(record, "relatedHabitatIds", habitatIds, "world.habitats", relativePath);
    assertIdRefs(record, "relatedRegionIds", regionIds, "world.regions", relativePath);
    assertIdRefs(record, "relatedMapFeatureIds", mapFeatureIds, "world.map_features", relativePath);
    assertProductionStageRefs(record, productionStageRefs, relativePath);
  }

  return {
    ok: true,
    resourceIds: records.map((record) => record.id).sort()
  };
}
