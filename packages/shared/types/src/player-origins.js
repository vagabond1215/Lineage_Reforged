export const BASE_PLAYER_RESOURCE_MAXIMA = {
    hp: 120,
    mp: 60,
    stamina: 100
};
const ZERO_RESOURCE_GROWTH = {
    hp: 0,
    mp: 0,
    stamina: 0
};
...
const DEFAULT_ATTRIBUTE_GROWTH_BIASES = {};
const ATTRIBUTE_KEYS = ["STR", "DEX", "AGI", "CON", "VIT", "WIS", "INT", "SPT", "CHA"];
const LINEAGE_ANCESTRY = {
    "lineage.human": {
        id: "lineage.human",
        name: "Human",
...
function averageResourceVectors(left, right) {
    return {
...
function scaleResourceVector(vector, factor) {
    return {
        hp: vector.hp * factor,
        mp: vector.mp * factor,
...
function createPlayableLineage(seed) {
    return {
        ...seed,
...
function createHybridLineage(params) {
    ...
    return createPlayableLineage({
        id: params.id,
        name: params.name,
        resourceBaseAdjustments: addResourceVectors(averageResourceVectors(left.resourceBaseAdjustments, right.resourceBaseAdjustments), params.resourceBaseModifiers ?? ZERO_RESOURCE_GROWTH),
        resourceGrowthPerLevel: addResourceVectors(averageResourceVectors(left.resourceGrowthPerLevel, right.resourceGrowthPerLevel), params.resourceGrowthModifiers ?? ZERO_RESOURCE_GROWTH),
        notes: [
...
export const PLAYER_LINEAGE_PROFILES = {
...
export const PLAYER_CLASS_PROFILES = {
    "class.explorer": {
        id: "class.explorer",
        name: "Scout",
...
export function getPlayerLineageProfile(lineageId) {
    return PLAYER_LINEAGE_PROFILES[lineageId];
}
export function getPlayerClassProfile(classId) {
    if (!classId) {
        return undefined;
    }
    return PLAYER_CLASS_PROFILES[classId];
}
function resolveLegacyGrowthLevels(input) {
...
export function resolvePlayerOriginProfile(coreData, progression) {
    ...
    const resolvedResourceMaxima = addResourceVectors(addResourceVectors(BASE_PLAYER_RESOURCE_MAXIMA, lineageBaseAdjustments), addResourceVectors(scaleResourceVector(lineageGrowth, levelGrowthSteps), scaleResourceVector(classGrowth, classLevel)));
    const notes = [
...
export function applyAttributeAdjustments(baseAttributes, adjustments) {
    return {
