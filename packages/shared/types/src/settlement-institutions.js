const LAND_VALUE_FACTOR = {
    low: 0.78,
    moderate: 1,
    high: 1.32
};
const LEGAL_STATUS_FACTOR = {
    clear_title: 1,
    disputed: 0.84,
    encumbered: 0.9,
    tax_delinquent: 0.8,
...
const DISTRICT_PRESTIGE_BY_TYPE = {
    central_market: "high",
    residential_low: "low",
...
const START_AUTHORITY_TIER_WEIGHT = {
    open: 1,
    temple: 2,
    frontier: 3,
    chartered: 4,
...
const SETTLEMENT_START_ACCESS_REGISTRY = [
    {
...
function getAllowedOwnersForDistrict(districtType) {
    switch (districtType) {
        case "central_market":
        case "storage_trade":
...
function mapGuildClass(guildKey) {
...
function chooseAuthorityId(settlement, authorityType) {
...
function scoreElementAffinity(settlement, region, locality) {
    const scores = {
        neutral: 0,
        light: 0,
        water: 0,
...
export function deriveLandAuthorityType(settlement) {
    ...
    if (settlement.settlementType === "outpost" ||
    ...
    if (settlement.guildPresence.length >= 3 &&
...
export function deriveDistrictOwnershipProfiles(params) {
    return params.districts.map((district) => {
        ...
        const prestigeLevel = district.landValue === "high" ? "high" : district.landValue === "moderate" ? basePrestige : "low";
...
export function deriveSettlementGuildInstances(params) {
    ...
    for (const entry of params.settlement.guildPresence) {
        const guildRecord = params.guildCatalog.find((candidate) => candidate.id === `guild.${entry.guildType}` || candidate.slug === entry.guildType);
        const guildKey = entry.guildType;
        const propertyIds = params.propertyRecords
        ...
        guilds.push({
            ...
            tradeInfluence: clamp(0.25 +
                params.settlement.tradeDependencyProfile.exportBias * 0.5 +
                params.settlement.infrastructureProfile.marketTier * 0.08, 0.2, 1.3),
            propertyIds,
            notes: [entry.notes]
        });
    }
    ...
    if ((params.settlement.infrastructureProfile.harborTier >= 2 ||
...
export function deriveSettlementReligion(params) {
    ...
    const topElements = Object.entries(affinity)
    ...
    const organizationIds = params.religionCatalog.organizations
        ...
        if (organization.category === "unbound") {
            return (params.settlement.survivalModel.hazardPressure >= 60 ||
    ...
    const preferredPlot = params.plots?.find((plot) => plot.tags.includes("near_water")) ??
        params.plots?.find((plot) => plot.tags.includes("high_ground")) ??
        null;
    const prestigeLevel = params.settlement.populationBand === "major" || params.settlement.administrativeRole !== "none" ? "high" : "moderate";
    const sites = organizationIds.map((organizationId, index) => {
        const siteType = organizationId === "religious_order.prismatic_enclave"
    ...
    return {
        ...
        organizationIds,
        sites,
        tensionNotes: organizationIds.includes("religious_order.prismatic_enclave") && organizationIds.includes("religious_order.the_unbound")
...
export function deriveSettlementCrystalReserves(params) {
    const affinity = scoreElementAffinity(params.settlement, params.region, params.locality);
    const rankedElements = Object.entries(affinity)
        .sort((left, right) => right[1] - left[1])
        .slice(0, 3)
        .map(([element]) => element);
    return rankedElements
        .map((element, index) => {
        const tier = index === 0 && params.settlement.populationBand !== "tiny" ? "crystal" : "shard";
        const crystal = params.crystalCatalog.find((record) => record.element === element && record.tier === tier) ??
            params.crystalCatalog.find((record) => record.element === "neutral" && record.tier === "shard");
        if (!crystal) {
            return null;
        }
        const capacityScale = 0.8 +
...
export function deriveSettlementMagicInfrastructure(params) {
    return params.magicCatalog.map((service) => {
        const hasGuild = service.requiredGuildTypes.length === 0 ||
            service.requiredGuildTypes.some((requiredGuild) => params.guilds.some((guild) => guild.guildKey === requiredGuild || guild.guildKey.endsWith(requiredGuild)));
        const hasReligion = service.requiredReligionOrganizationIds.length === 0 ||
        ...
        const passesInfrastructure = infra.roadTier >= service.requiredInfrastructure.roadTier &&
        ...
        const matchingCrystals = params.crystalReserves.filter((crystal) => service.preferredCrystalTiers.includes(crystal.tier) && service.allowedElements.includes(crystal.element));
...
function deriveRequiredStartAuthorityTier(params) {
    const isHighTierCity = params.settlement.populationBand === "major" ||
    ...
    if (params.settlement.identityTags.includes("monastery") ||
        params.settlement.purposeTags.includes("pilgrimage")) {
        return "temple";
    }
    return "open";
}
function getSettlementStartAccessSpecificity(row) {
    return ((row.restrictedSettlementTypes?.length ?? 0) +
        (row.disallowedOriginContexts?.length ?? 0));
}
function isSettlementStartAccessRowApplicable(row, settlement, authorityTier) {
    if (row.authorityTier !== authorityTier) {
        return false;
    }
    if (row.restrictedSettlementTypes &&
...
function getApplicableSettlementStartAccessRows(params) {
    ...
    return SETTLEMENT_START_ACCESS_REGISTRY.filter((row) => isSettlementStartAccessRowApplicable(row, params.settlement, params.authorityTier));
}
function resolveSettlementStartAccessRow(params) {
    ...
    matches.sort((left, right) => {
        const specificityDelta = getSettlementStartAccessSpecificity(right.row) -
            getSettlementStartAccessSpecificity(left.row);
        if (specificityDelta !== 0) {
            return specificityDelta;
        }
        const authorityDelta = START_AUTHORITY_TIER_WEIGHT[right.row.authorityTier] -
...
export function deriveSettlementStartAccess(params) {
    ...
    const allowedBackstoryIds = [];
    let accessStatus = "allowed";
    let spawnMode = "rented_lodging";
    let lodgingType = "rented_room";
    let lawfulStanding = "ordinary";
    let sponsorCategory = "civic_hospitality";
    const notes = [];
...
function resolvePropertyOwner(params) {
    ...
    if (params.district.districtType === "civic_religious") {
        return {
            ...
            ownerId: params.building?.category === "civic"
            ...
            operatorId: params.building?.category === "civic"
                ? `authority.${params.settlement.id}.civic_council`
                : `temple.${params.settlement.id}.religious_order`,
            holdingType: "owned",
            legalStatus: "clear_title"
    ...
    if (params.district.districtType === "central_market" || params.district.districtType === "storage_trade") {
        if (params.settlement.guildPresence.length > 0) {
            ...
            return {
                ...
                ownerId: params.landAuthorityType === "guild_controlled"
    ...
    if (params.district.districtType === "industrial_production") {
        return {
            ...
            ownerId: params.settlement.guildPresence.length > 0
            ...
            operatorId: params.settlement.guildPresence.length > 0
                ? `guild.${params.settlement.id}.${params.settlement.guildPresence[0].guildType}`
                : `company.${params.settlement.id}.${params.district.districtType}`,
            holdingType: "owned",
            legalStatus: "clear_title"
...
export function derivePropertyOwnershipRecords(params) {
    ...
    return params.plots
        ...
        const legalStatus = plot.state === "abandoned" || plot.state === "dilapidated"
            ? "condemned"
            : plot.state === "vacant"
                ? "clear_title"
        ...
        const repairScaleFactor = Number((landFactor * sizeScore * (1.1 + Math.max(0, 0.85 - condition)) * (legalStatus === "condemned" ? 1.2 : 1)).toFixed(2));
