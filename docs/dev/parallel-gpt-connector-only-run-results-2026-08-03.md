...
Provide one durable discovery and routing index for the first ten sequential connector-only evidence refreshes.
...
- inspected master `91bd8c2c89c85fb9ea7257b2c96b68ab41231b04` through the GitHub Connector;
- updated only the branch's one unique audit document;
- changed no production, test, schema, content, asset, dependency, current-prompt, current-output, current-handoff, roadmap, planning-anchor, backlog, or active-route file;
- ran no local tests, builds, typechecks, linters, simulations, or generated-output checks;
- remained isolated and unmerged as `CANDIDATE_INTEGRATION`;
- installed an explicit named consumer and review trigger.
...
A future run must inspect the relevant branch document when:
...
3. it claims the domain is mature, engine-owned, replay-safe, presentation-ready, or deletion-ready;
4. it proposes integrating, superseding, retiring, or deleting the branch;
5. a representative-loop or maturity-band audit relies on the domain.

The completion report must state which entries were applicable and whether they were inspected. Unrelated entries may be omitted.

## Authority And Integration Rule

The branch documents are evidence, not current execution authority. Current prompt, handoff, output, and newer focused decisions take precedence.

Do not merge all ten documents as a batch. At a named consumer or dedicated coordinator:

- resolve the exact branch head;
- compare against then-current master;
- classify each finding as current, superseded, narrowed, or historically useful;
- integrate, re-author, hold, or retire independently;
- update `docs/dev/branch-disposition-register.md` once from complete evidence;
- verify equivalent preservation before branch deletion.

## Current Active-Route Relationship

The active historical recovery-fork authority decision should inventory and preserve these branch dispositions, but must not integrate or rewrite them inside the active `0.6.9` decision.

## Completed Second Wave

The second ten connector-only audits are complete and indexed separately at:

`docs/dev/parallel-gpt-connector-only-second-wave-results-2026-08-03.md`

Second-wave result-index commit:
...
Future work must inspect semantically applicable entries across both indexes. Neither wave is active execution authority, and neither should be merged as a batch.
