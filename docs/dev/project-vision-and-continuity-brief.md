...
- Conditional successor: the unversioned Ashen Reef survey receipt-foundation decision, only after accepted `0.6.9.8`.
...
- `Version 0.6.9` is implemented, but parent acceptance remains pending. Revised `0.6.9.7` reports the additional initial-resolution, restart-duplicate, effect-provenance, continuity, destination-source, and source-before-fork defects repaired; active `0.6.9.8` must independently accept them.
...
1. run active `Version 0.6.9.8` as an independent parent acceptance audit of implemented `0.6.9.7`;
...
- Normal target authority no longer treats ordinary HP zero as terminal. Revised `0.6.9.7` is implemented; parent `0.6.9` remains unaccepted until independent `0.6.9.8` accepts the recovery findings.
