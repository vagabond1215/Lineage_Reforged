...
- Latest landed version: `Version 0.5.89 - Known Spell Ownership Helpers`
- Next recommended version: `Version 0.5.90 - Known Spell Validation Helpers`
...
| `0.5.90` | Known Spell Validation Helpers | Codex Local | Pure validation helpers + focused tests | Next. Add collection validation, duplicate id checks, and minimal training-event evidence validation. | No casting, UI, save schema migration, catalyst behavior, scroll/tome behavior, Magic Legacy power, or broader ownership scopes. |
...
| Magic Runtime Readiness Audit | Promoted into `docs/design/known-spell-ownership-plan.md`; consumed by `0.5.88`; retained as the source-detail reference for `0.5.90` known-spell validation helpers. |
