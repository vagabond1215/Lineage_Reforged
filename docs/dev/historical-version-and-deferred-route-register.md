...
| `0.6.6` | Monster, Ecology, And Loot Static Content Expansion | Active parent primary; authoring still fail-closed | Begin only after `0.6.6.5` completes validation and restores the exact parent prompt. | `docs/dev/held-0.6.6-monster-ecology-loot-prompt.md` and blob `42014541c15d2d7ccc01f43dd8b0a4fa6fbf8769` |
...
| `0.6.6.5` | Workspace Typecheck Baseline Classification And BOM Acceptance | Active support suffix | Classify the unchanged 173-diagnostic workspace audit correctly, run deferred green gates, restore exact `0.6.6`, and stop. | `docs/dev/current-codex-prompt.md` |
...
| Validation/typecheck | known-failing audit with separate cleanup tracks | Approved command matrix, source map, and blocker triage; current 173-diagnostic baseline | Dedicated JSON import, environment typing, root-config/JSX, target/lib, module, and strict optional cleanup | Explicit tooling/typecheck route | Do not mix the broad backlog into `0.6.6.5`; block only on new or changed diagnostics | `docs/design/validation-command-matrix-plan.md` |
| Static world/settlement | expanded-and-validated with acceptance active | Accepted `0.6.4`; exact climate/population repair commit `232d3c2f`; focused `5/5`; lint `67` | Baseline-aware typecheck classification, parent baseline, diff/hygiene, restoration | Accepted `0.6.6.5` | Complete validation only; no further implementation | `docs/dev/current-codex-prompt.md` |
...
| Static monster/ecology/loot | active parent primary; support validation required | Exact matrix and closed references; support implementation landed | Complete `0.6.6.5`, restore exact parent prompt, then run package | Accepted `0.6.6.5` | Validation and restoration first; no parent authoring in support pass | `docs/dev/current-codex-prompt.md` |
