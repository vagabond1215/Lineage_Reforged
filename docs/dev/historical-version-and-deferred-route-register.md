...
| `0.6.6` | Monster, Ecology, And Loot Static Content Expansion | Active parent primary; authoring still fail-closed | Exact content target remains accepted. Begin only after `0.6.6.4` completes every validation and restores the exact parent prompt. | `docs/dev/held-0.6.6-monster-ecology-loot-prompt.md` and blob `42014541c15d2d7ccc01f43dd8b0a4fa6fbf8769` |
| `0.6.6.1` | UTF-8 BOM Test-Harness Repair | Repair implementation landed; acceptance pending | Commit `66f12fd6f649f8f218f7f49fc721a8fe545a7a01` changes the two intended test readers. Final acceptance is folded into `0.6.6.4`. | `docs/dev/current-codex-output.md` |
| `0.6.6.2` | BOM Repair Post-Validation And Parent Prompt Restoration | Completed fail-closed attempt; superseded | Repair scope and content identity passed, but the focused command stopped at `4/5` and exposed the initial region climate contract mismatch. No files changed and no parent prompt was restored. | `docs/dev/current-codex-output.md` |
| `0.6.6.3` | Region Climate Tendencies Contract Repair And BOM Acceptance | Partial implementation followed by fail-closed stop; superseded | Schema commit `56932eec` and focused climate assertion commit `e71f8f6b` landed. Validation remained `4/5`; five scalar records and a stale population-capacity assertion required broader authorization. No parent prompt was restored. | `docs/dev/current-codex-output.md` |
| `0.6.6.4` | Region Climate Data Migration, Population Assertion Repair, And BOM Acceptance | Active support suffix | Migrate exactly five scalar climate values to singleton arrays, repair the population assertion and static types, rerun all acceptance gates, restore exact `0.6.6`, and stop. | `docs/dev/current-codex-prompt.md` |
...
| Static world/settlement | expanded-and-validated with bounded repair active | Accepted `0.6.4`; climate schema and focused array assertion partially repaired | Exact five-record shape migration, stale population assertion repair, static-type alignment, and validation | Accepted `0.6.6.4` | Complete the bounded support pass; no parent content authoring | `docs/dev/current-codex-prompt.md` |
...
| Static monster/ecology/loot | active parent primary; support validation required | Exact matrix and closed references; BOM repair landed; two partial climate contract commits landed | Complete `0.6.6.4`, restore exact parent prompt, then run package | Accepted `0.6.6.4` | Migration and validation first; no monster/ecology/loot authoring in support pass | `docs/dev/current-codex-prompt.md` |
