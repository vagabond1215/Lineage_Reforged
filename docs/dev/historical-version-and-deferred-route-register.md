...
| `0.6.9` | Normal Stakes Campaign Persistence Foundation | Implemented; independent acceptance pending | Revised `0.6.9.7` is implemented at `ba35dacd`; parent acceptance remains pending active `0.6.9.8`. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
...
| `0.6.9.8` | Initial Defeat And Durable Recovery Completion Acceptance Audit | Active support audit | Independently reproduce the repaired contract, callers, restart/failure boundaries, version-6 persistence, and validation before accepting parent `0.6.9`. | `docs/dev/current-codex-prompt.md` |
...
| `Normal Defeat Recovery Continuity And Destination Provenance Contract Decision` | Complete; accepted | `DECISION_ACCEPTED_REPAIR_REAUTHORIZED`; its exact contract is implemented by `0.6.9.7`, parent remains unaccepted, and `0.6.9.8` is active. | `docs/design/normal-defeat-recovery-continuity-and-destination-provenance-contract-decision.md` |
...
| Activity resolution reuse | survey/save contracts and implemented Normal persistence repairs | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, exact survey owner contract, implemented Normal persistence foundation, accepted recovery continuity/provenance contract, and implemented `0.6.9.7` repair | Independently audit through active `0.6.9.8`, then decide the persisted survey occurrence/result/consequence receipt foundation | Independent acceptance in `0.6.9.8` | Block survey receipt and implementation until the parent persistence chain is independently accepted | `docs/dev/current-codex-prompt.md` |
