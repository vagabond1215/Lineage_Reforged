...
| Activity resolution reuse | survey owner and minimum save contracts accepted; dependency closure active | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, exact survey owner contract, and minimum Normal-only identity/migration/ledger/publication contract | Atomic Normal activation plus nonterminal defeat, first-mutation continuity admission, and campaign-publication-before-account-value ordering; then exact implementation and survey layers | Complete the active unversioned dependency-closure decision | Keep general competence/difficulty/familiarity separate; survey and save implementation remain `NO_PACKAGE` | `docs/design/ashen-reef-survey-minimum-save-identity-and-accepted-state-publication-decision.md` |
