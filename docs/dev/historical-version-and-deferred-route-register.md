...
| `0.6.9` | Normal Stakes Campaign Persistence Foundation | Complete and accepted | Installed `0.6.9.11` independently accepted `cbad987` under bounded Model C with the complete parent gate. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
| `0.6.9.1` | Normal Stakes Campaign Persistence Foundation Acceptance Audit | Completed; original findings repaired, parent reopened | Historical audit remains evidence; later post-repair inspection supersedes its accepted status. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
| `0.6.9.2` | Normal Campaign Publication Recovery Repair | Completed support suffix; partial acceptance evidence | Repaired six original findings and immutable address verification, but did not close actual launcher retry identity, same-slot recovery collision, or reachable validated pending recovery completion. | `docs/design/normal-campaign-new-game-retry-and-recovery-collision-audit.md` |
| `0.6.9.3` | New-Campaign Retry, Slot-Recovery Collision, And Pending-Defeat Repair Completion | Implemented; audited with repair required | Preserved 23/23 focused tests, 130/130 prescribed tests, and the 209-module build, but did not close all completion-authority boundaries. | `docs/dev/current-codex-output.md` |
| `0.6.9.4` | Normal Campaign Retry And Recovery Completion Acceptance Audit | Completed support audit; repair required | Independently reproduced three blocking defects: first-pending selection, ruin-backed current-location acceptance, and absent repair-ledger provenance. | `docs/dev/current-codex-output.md` |
| `0.6.9.5` | Pending-Defeat Completion Authority And Provenance Repair | Implemented; pending independent parent audit | Reports 26/26 focused tests, 133/133 prescribed tests, a passing build, unique pending-receipt admission, exact settlement authority, and one superseding ledger entry. | `docs/dev/current-codex-output.md` |
| `0.6.9.6` | Pending-Defeat Completion Authority And Provenance Acceptance Audit | Completed support audit; repair required | Preserved 26/26 focused tests, 133/133 prescribed tests, and the build, but proved three remaining initial-resolution, restart-duplicate, and effect-provenance defects. | `docs/dev/current-codex-output.md` |
| `0.6.9.7` | Initial Defeat Authority, Durable Duplicate, And Effect-Provenance Repair | Implemented; pending independent audit | Implemented all nine findings at `ba35dacd852304cd0804b131c8d3045c1f74b755`; reported 32/32 focused, 139/139 prescribed, 10/10 adversarial, passing build, and zero changed-file TypeScript diagnostics. | `docs/dev/current-codex-output.md` |
| `0.6.9.8` | Initial Defeat And Durable Recovery Completion Acceptance Audit | Complete support audit; repair required | Confirmed completion-lineage substitution and deep-descendant replay defects. | `docs/dev/version-0.6.9.8-pre-audit-completion-continuity-lineage-review-2026-08-02.md` |
...
| `0.6.10` | Ashen Reef Survey Advancement Authority | Implemented, repaired twice, and accepted | Commit `008db9c93eb8818aea51652be07fd196df41c45f` implements the bounded persisted four-stage package; support repairs `59af92629a79e95fa20247959159e336a8dbc88e` and `07c57392c8078927e4f9e12efe18d8d89bb1fc70` address all eight findings; independent `0.6.10.5` accepted the parent at `950e851446fb75bfbdb717d0ea33e33ec2907d4a`. | `docs/design/ashen-reef-survey-advancement-authority-acceptance-audit.md` |
| `0.6.10.1` | Ashen Reef Survey Advancement Acceptance Audit | Complete support suffix; `REPAIR_REQUIRED` | Preserved the positive `167/167` matrix while proving deep-validation, correction-evidence, projection-order, caller-identity, explicit-non-effect, and skill-presentation gaps; made no production changes. | `docs/design/ashen-reef-survey-advancement-authority-acceptance-audit.md` |
| `0.6.10.2` | Ashen Reef Survey Advancement Authority Repair | Complete support suffix; `IMPLEMENTED_PENDING_POST_REPAIR_AUDIT` | Implemented all six audited findings at `59af92629a79e95fa20247959159e336a8dbc88e`; preserved the positive contract matrix and installed separate production-read-only `0.6.10.3`. | `docs/design/ashen-reef-survey-advancement-authority-acceptance-audit.md` |
| `0.6.10.3` | Ashen Reef Survey Advancement Post-Repair Acceptance Audit | Complete support suffix; `REPAIR_REQUIRED` | Preserved the green repaired matrix but proved owner-derived progression can be forged into duplicate authority and byte-correct projection placement drift is undiscoverable across restart; made no production/test changes and issued no representative classification. | `docs/design/ashen-reef-survey-advancement-authority-acceptance-audit.md` |
| `0.6.10.4` | Ashen Reef Survey Progression Coherence And Projection Placement Repair | Complete support suffix; `IMPLEMENTED_PENDING_REAUDIT` | Recomputes progression from retained owner inputs and repairs byte-correct survey projection placement with opaque-slot preservation at `07c57392c8078927e4f9e12efe18d8d89bb1fc70`; preserves the retained matrix and installs separate production-read-only `0.6.10.5`. | `docs/design/ashen-reef-survey-advancement-authority-acceptance-audit.md` |
...
| `0.6.11` | Ashen Reef Survey Ordinary Reachability And Representative Loop Evidence | Implemented primary; pending parent audit | Implemented the bounded authored definition, offer/access owners, direct origin correction, versioned presentation compatibility, and injection-free creator-to-restart evidence at `3ca23d6864541a899ea61a6bf26257665f754e78`; it may not self-accept. | `docs/design/ashen-reef-survey-ordinary-reachability-implementation-package-decision.md` |
| `0.6.11.1` | Ashen Reef Survey Ordinary Reachability And Representative Loop Acceptance Audit | Active installed support audit | Production-read-only audit of `0.6.11`; independently decide parent and representative-loop acceptance without repair in the same run. | `docs/dev/current-codex-prompt.md` |
...
| `Ashen Reef Survey Ordinary Reachability And Representative Loop Dependency Closure Decision` | Complete; `NO_PACKAGE` | Proved downstream travel/activity/persistence seams reusable but found no authoritative exact quest offer/journal-admission writer or Ashen known-location/access grant; prohibited inference across conflicting demo, runtime, and authored-content facts. | `docs/design/ashen-reef-survey-ordinary-reachability-and-representative-loop-dependency-closure-decision.md` |
| `Ashen Reef Soundings Authored-Canon Decision` | Complete; `AUTHORED_INPUT_ACCEPTED` | Settled the Starfall quest/place identity, issuer, premise, one-time/non-expiring posture, four stages, deferred rewards, and quest-acceptance access occurrence. | `docs/design/ashen-reef-soundings-authored-canon-decision.md` |
| `Ashen Reef Survey Ordinary Reachability Implementation Package Decision` | Complete; `PACKAGE_READY` | Selected current-band primary `0.6.11` with one bounded authored/schema, offer, access, travel-origin, presentation-compatibility, and representative-evidence package. | `docs/design/ashen-reef-survey-ordinary-reachability-implementation-package-decision.md` |
...
| Activity resolution reuse | bounded survey parent accepted; ordinary reachability implemented pending audit | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, accepted `0.6.9`, accepted `0.6.10`, accepted Ashen canon, reusable travel-owned survey activation, sufficient version-7 persistence, and implementation `3ca23d6864541a899ea61a6bf26257665f754e78` | Independently audit the bounded ordinary-reachability owners and injection-free evidence | Active `Version 0.6.11.1 - Ashen Reef Survey Ordinary Reachability And Representative Loop Acceptance Audit` | Keep turn-in/rewards, geographic Knowledge/map behavior, generic quest/activity/travel infrastructure, other Stakes modes, and direct `0.7.0` entry outside `0.6.11.1` | `docs/dev/current-codex-prompt.md` |
