...
| `0.6.9` | Normal Stakes Campaign Persistence Foundation | Implemented; acceptance withheld pending repair | Atomic campaign rules/identity, publication/migration, first-mutation continuity, nonterminal Normal defeat, legacy repair, and publication-keyed account consumers pass the existing 120-test group and RPG UI build, but four recovery-authority defects remain. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
| `0.6.9.1` | Normal Stakes Campaign Persistence Foundation Acceptance Audit | Completed; repair required | Existing 120-test/build evidence passed, but four recovery-authority defects withheld parent acceptance. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
| `0.6.9.2` | Normal Campaign Publication Recovery Repair | Active support suffix | Repair address-after-head recovery, durable account-consumer repair, separately loaded legacy HP-zero repair, and missing/invalid/closed control rejection. | `docs/dev/current-codex-prompt.md` |
