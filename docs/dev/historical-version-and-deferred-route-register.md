...
| `0.6.9` | Normal Stakes Campaign Persistence Foundation | Implemented; parent acceptance withdrawn | `0.6.9.2` closed the original six findings, but real new-campaign retry, same-slot recovery collision, and reachable pending recovery completion remain open. | `docs/design/normal-campaign-new-game-retry-and-recovery-collision-audit.md` |
...
| `0.6.9.3` | New-Campaign Retry, Slot-Recovery Collision, And Pending-Defeat Repair Completion | Active support implementation | Must close the exact remaining parent defects and report `IMPLEMENTED_PENDING_PARENT_AUDIT`; it may not accept the parent or advance to survey work. | `docs/dev/current-codex-prompt.md` |
| `0.6.9.4` | Normal Campaign Retry And Recovery Completion Acceptance Audit | Mandatory successor when `0.6.9.3` is green | Independent inspection and validation only unless it proves a new exact repair is required; only this route may restore parent acceptance. | `docs/dev/current-codex-prompt.md` |
...
| Activity resolution reuse | survey/save contracts and partial `0.6.9` persistence authority implemented | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, exact survey owner contract, and implemented Normal persistence foundation | Implement `0.6.9.3`, independently accept it through `0.6.9.4`, then decide the persisted survey occurrence/result/consequence receipt foundation | Parent acceptance through `0.6.9.4` | Block survey receipt and implementation until the parent persistence chain is independently accepted | `docs/dev/current-codex-prompt.md` |
