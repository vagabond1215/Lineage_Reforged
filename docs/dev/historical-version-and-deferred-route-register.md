...
| `0.6.9` | Normal Stakes Campaign Persistence Foundation | Implemented; acceptance withheld pending repair | Existing 120-test/build evidence is green, but six recovery/admission authority defects and missing address-to-immutable-artifact verification remain. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
| `0.6.9.1` | Normal Stakes Campaign Persistence Foundation Acceptance Audit | Completed; repair required | Audit and follow-up source inspection withheld parent acceptance for post-head address recovery, durable account repair, separately loaded HP-zero repair, missing control rejection, `recovery_pending` admission, duplicate replay, and address/artifact consistency. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
| `0.6.9.2` | Normal Campaign Publication Recovery Repair | Active support suffix | Repair all six proven defects and verify playable addresses against immutable artifact authority without broadening into a generic transaction or replay framework. | `docs/dev/current-codex-prompt.md` |
...
| Activity resolution reuse | survey/save contracts accepted; `0.6.9.1` requires persistence repair | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, exact survey owner contract, minimum save contract, and implemented persistence foundation | Repair and accept all current `0.6.9` recovery/admission defects; then classify exact survey receipt and command layers | Complete `0.6.9.2` and fresh acceptance | Keep general competence/difficulty/familiarity separate; do not implement survey during parent repair | `docs/dev/current-codex-prompt.md` |
