...
| Activity resolution reuse | accepted repository audit; survey prerequisite active | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, and post-foundation classification | Exact Ashen Reef survey command/result, occurrence/receipt, affected-owner, persistence/replay/correction, preview, and accepted-only UI decision | Complete active unversioned owner-contract decision | Keep general competence/difficulty/familiarity separate; implementation remains `NO_PACKAGE` | `docs/design/post-lethal-process-static-foundation-next-capability-classification-gate.md` |
