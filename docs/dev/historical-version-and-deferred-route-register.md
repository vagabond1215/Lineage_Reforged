...
| `0.6.9` | Normal Stakes Campaign Persistence Foundation | Implemented; parent repair required | `0.6.9.3` closed the launcher retry and slot-collision findings, but `0.6.9.4` proved pending-defeat ambiguity, unsafe current-location authority, and missing repair-ledger provenance. | `docs/design/normal-stakes-campaign-persistence-foundation-acceptance-audit.md` |
...
| `0.6.9.6` | Pending-Defeat Completion Authority And Provenance Acceptance Audit | Active support audit | Must independently decide the complete receipt, destination, provenance, effect, publication, duplicate, restart, and preservation boundaries. | `docs/dev/current-codex-prompt.md` |
...
| Activity resolution reuse | survey/save contracts and implemented `0.6.9.5` persistence repair | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, exact survey owner contract, and implemented Normal persistence foundation | Independently audit `0.6.9.5` through `0.6.9.6`, then decide the persisted survey occurrence/result/consequence receipt foundation | Parent acceptance through `0.6.9.6` | Block survey receipt and implementation until the parent persistence chain is independently accepted | `docs/dev/current-codex-prompt.md` |
