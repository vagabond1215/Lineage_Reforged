...
| `0.6.10` | Ashen Reef Survey Advancement Authority | Implemented and repaired; pending independent acceptance | Commit `008db9c93eb8818aea51652be07fd196df41c45f` implements the bounded persisted four-stage package; support repair `59af92629a79e95fa20247959159e336a8dbc88e` closes the six audited defects pending `0.6.10.3`. | `docs/design/ashen-reef-survey-occurrence-result-and-consequence-receipt-foundation-decision.md` |
...
| `0.6.10.3` | Ashen Reef Survey Advancement Post-Repair Acceptance Audit | Active support suffix | Independently reproduce every repaired negative case and retained positive contract before deciding parent and representative-loop posture; production remains read-only. | `docs/dev/current-codex-prompt.md` |
...
| Activity resolution reuse | bounded survey parent pending post-repair audit | Selection, trial, quest, Knowledge, crafting, magic, occurrence foundations, exact survey owner/receipt decisions, accepted `0.6.9`, implemented `0.6.10`, `0.6.10.1` findings, and `0.6.10.2` repair | Independent parent and representative-loop acceptance after the six repaired contracts are reproduced | Active production-read-only `Version 0.6.10.3 - Ashen Reef Survey Advancement Post-Repair Acceptance Audit` | Keep rest, turn-in, generic activity, rewards, geographic Knowledge implementation, repair inside the audit, and direct `0.7.0` entry outside the audit | `docs/dev/current-codex-prompt.md` |
