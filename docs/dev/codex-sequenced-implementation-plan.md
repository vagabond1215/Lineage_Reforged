...
This file gives Codex a stable ordered queue after `Version 0.5.91 - Known Spell Acquisition Evidence Helpers` added pure training-event acquisition evidence helper boundaries.
...
Latest landed magic evidence slice:
...
| 14 | `0.5.92` | Known Spell Read-Only Projection | Codex 5.5 Local | `docs/dev/current-codex-output.md` | pure projection + focused tests | Next | Read-only projection only; no acquisition mutation, casting, UI, save schema migration, or broader routes/scopes. |
...
- Do not add generated UI output during known-spell read-only projection work.
- Do not import Node-only content loaders or engine barrels into browser-facing UI files.
- Do not mix known-spell read-only projection with combat math rewrites, active spell execution, ammo behavior, ranged balancing, broad equipment slot metadata, shield/armor training, hybrid/improvised weapon policy, or UI implementation.
- Do not begin runtime magic until known-spell ownership helpers, validation, acquisition evidence helpers, read-only projection, conduit/catalyst/control policy, and blocked-hook tests are explicitly implemented in narrow slices.
