...
This file gives Codex a stable ordered queue after `Version 0.5.90 - Known Spell Validation Helpers` added pure collection validation, duplicate known-spell id detection, and minimal training-event evidence validation.
...
Latest landed magic validation slice:
...
| 13 | `0.5.91` | Known Spell Acquisition Evidence Helpers | Codex 5.5 Local | `docs/dev/current-codex-output.md` | pure acquisition evidence helpers + focused tests | Next | Training-event evidence helper boundaries only; no acquisition mutation, casting, UI, save schema migration, or broader routes/scopes. |
...
- Do not add generated UI output during known-spell acquisition evidence helper work.
- Do not import Node-only content loaders or engine barrels into browser-facing UI files.
- Do not mix known-spell acquisition evidence helpers with combat math rewrites, active spell execution, ammo behavior, ranged balancing, broad equipment slot metadata, shield/armor training, hybrid/improvised weapon policy, or UI implementation.
- Do not begin runtime magic until known-spell ownership helpers, validation, acquisition evidence helpers, conduit/catalyst/control policy, and blocked-hook tests are explicitly implemented in narrow slices.
