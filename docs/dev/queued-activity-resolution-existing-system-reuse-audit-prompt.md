# Queued Activity Resolution Existing-System Reuse Audit Prompt

## Hold Status

This file is a future documentation-only hold. It does not control current execution and must not replace `docs/dev/current-codex-prompt.md` until all prerequisites below are accepted and current coordination explicitly activates it.

Do not run this prompt before:

1. accepted revised `Version 0.6.5 - Item, Material, And Recipe Static Content Expansion`;
2. accepted `Version 0.6.6 - Monster, Ecology, And Loot Static Content Expansion`;
3. accepted `Version 0.6.7 - Cross-Content Coherence And Coverage Audit` and its required temporary-artifact decisions;
4. accepted Geographic Knowledge Taxonomy And Location Recognition Contract Plan;
5. a live route check confirming that no newer authority supersedes this hold.
...
Do not activate this queued prompt merely by editing it.
