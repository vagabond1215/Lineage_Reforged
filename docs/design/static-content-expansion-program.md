...
Status: approved static-content milestone; no content implemented by this decision
...
| Settlement districts | `packages/content/base/world/settlement_districts.json` — 2 | 2 active, both Highcrown | `packages/schemas/world/settlement-district.schema.json` | `tools/content-lint/settlement-districts.mjs`; registered | active parent settlement, unique slug/id, supported district type | Knowledge/presentation authority; no simulation | `ready-for-large-static-expansion` | Parent and status coherence; active Knowledge subjects require active records. |
| Settlement sites | `packages/content/base/world/settlement_sites.json` — 2 | 2 active, both Highcrown | `packages/schemas/world/settlement-site.schema.json` | `tools/content-lint/settlement-sites.mjs`; registered | parent settlement, optional district, supported site type | Knowledge/presentation authority; no simulation | `ready-for-large-static-expansion` | Parent/district and status coherence; no generic POI substitution. |
| Regions | `packages/content/base/world/regions.json` — 41 | established hierarchy | `packages/schemas/world/region.schema.json` | semantic checks in `tools/content-lint/index.mjs`; registered | world maps and parent-region hierarchy | geography/travel/world presentation | `ready-with-reference-constraints` | New regions require map and hierarchy evidence; `0.6.4` should reuse current regions unless its matrix proves a gap. |
...
| Semantic map features | `packages/content/base/world/map_features.json` — 2 | 2 planned: one mountain range, one river | `packages/schemas/world/map-feature.schema.json` | `tools/content-lint/map-features.mjs`; registered | region/settlement/world-map anchors and visual references | static semantic reference only | `ready-for-large-static-expansion` | Must represent named geographic features with valid anchors, not a generic POI catalog. |
| Visual world-map aggregate | `packages/content/base/world/world_map_features.json` — 1 | one geometry/reference record | `packages/schemas/world/world-map-feature.schema.json` | semantic checks in `tools/content-lint/index.mjs`; registered | world maps, image dimensions, regions, climate and biome zones | map rendering/reference | `authored-input-gated` | Requires authoritative cartography/geometry input. Do not fabricate geometry during `0.6.4`. |
...
| Knowledge snippets | `packages/content/base/player/knowledge_snippets.json` — 16 | settlement/district/site, region, flora/fauna, mineral, religion/site subjects | `packages/schemas/player/knowledge_snippet.schema.json` | `tools/content-lint/knowledge-snippets.mjs`; registered | domain registry and live subject authorities | descriptive Knowledge content | `ready-for-large-static-expansion` | Expand alongside source canon; snippets remain informational and non-granting. |
| Services | `packages/content/base/civilization/services.json` — 5 | 5 planned | `packages/schemas/civilization/service.schema.json` | `tools/content-lint/services.mjs`; registered | provider anchor types and building service functions | vocabulary only; no availability owner | `paused` | Post-registration audit found no immediate expansion need. `0.6.4` may reference existing vocabulary descriptively but must not add availability, providers, prices, stock, or effects. |
...
`0.6.4` therefore targets exactly 12 districts, 18 sites, 6 semantic map features, and 12 General Lore snippets across nine existing settlements. It adds no settlement, region, locality, visual geometry, service, resource, or commodity records unless pre-authoring evidence proves that a required reference cannot be represented otherwise; any such exception must be documented before editing and keep the total package coherent.
...
| --- | --- | --- |
| `0.6.4` | World And Settlement Static Content Expansion | Target matrix completed; parent/region/locality/map/Knowledge closure; schemas, focused settlement/map/Knowledge tests, and normal lint pass; no generic POI, geometry fabrication, paused-catalog expansion, or runtime claim. |
