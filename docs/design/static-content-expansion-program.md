...
Status: static program `0.6.4`-`0.6.8.1` complete and accepted; Normal persistence `0.6.9` accepted; Ashen Reef survey parent `0.6.10` accepted; Soundings canon and ordinary-reachability `0.6.11` accepted through `0.6.11.1`; `DEV-0.7.0` accepted; current band `DEV-0.7.x`; Game `0.1.0-prealpha` / `INTEGRATED_LOOP`; playability priority selected; Soundings authored return/submission/payment terms decision active
