...
| `0.5.246` | First People And NPC Content Seed Plan | Completed docs-only; recommended people-only first and deferred NPC overlays. |
| `0.5.247` | First People And NPC Content Seed | Completed/deferred; no safe canonical named-person evidence, no live people/NPC content, and no normal lint registration. |
| `0.5.248` | First Polity Content Seed Plan | Completed docs-only; selected tiny planned-only future seed posture. |
...
Safest immediate next local Codex run: `Version 0.5.249 - First Polity Content Seed`, conditional on explicit live-content authorization and fresh candidate audit.
...
It should use the current polity seed plan as its source and create only a tiny planned-only content seed if fresh audit reconfirms at least one approved candidate.
