...
- Nine temporary Deep Research artifacts remain in `docs/dev`. The settlement, crafting, and combat artifacts were retired by `0.5.218` through `0.5.220`; each remaining artifact has been consumed by a permanent decision but retains detail for one named follow-up.
...
Safest immediate next local Codex run: `Version 0.5.221 - Weapon And Armor Profile Schema Decision`.
...
It should remain documentation-only, define future weapon/armor profiles against current item metadata, update the temporary item artifact's retirement status, and make no schema, validator, content, test, runtime, UI, storage, or gameplay change.
