...
- Five temporary Deep Research artifacts remain in `docs/dev`. The settlement, crafting, combat, item/equipment, quest/event/Chronicle, NPC/social, and magic/Knowledge/study artifacts were retired by `0.5.218` through `0.5.224`; each remaining artifact has been consumed by a permanent decision but retains detail for one named follow-up.
...
Safest immediate next local Codex run: `Version 0.5.225 - Polity Schema Decision`.
...
It should remain documentation-only, define exact polity schema posture from live authorities, update the temporary civic artifact's retirement status, and make no schema, validator, content, test, runtime, UI, storage, or gameplay change.
