...
- Eight temporary Deep Research artifacts remain in `docs/dev`. The settlement, crafting, combat, and item/equipment artifacts were retired by `0.5.218` through `0.5.221`; each remaining artifact has been consumed by a permanent decision but retains detail for one named follow-up.
...
Safest immediate next local Codex run: `Version 0.5.222 - Quest Objective And Condition Schema Decision`.
...
It should remain documentation-only, reconcile embedded quest objectives/conditions across current authorities, update the temporary quest/event artifact's retirement status, and make no schema, validator, content, test, runtime, UI, storage, or gameplay change.
