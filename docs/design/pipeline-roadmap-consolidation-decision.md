...
- Ten temporary Deep Research artifacts remain in `docs/dev`. The settlement and crafting artifacts were retired by `0.5.218` and `0.5.219`; each remaining artifact has been consumed by a permanent decision but retains detail for one named follow-up.
...
Safest immediate next local Codex run: `Version 0.5.220 - Monster Record Schema Decision`.
...
It should remain documentation-only, audit the existing monster schema and runtime-field boundary, update the temporary combat artifact's retirement status, and make no schema, validator, content, test, runtime, UI, storage, or gameplay change.
