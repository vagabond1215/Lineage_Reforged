...
The roadmap top override and the sequenced-plan current anchor are refreshed through the accepted care-capability contract, with the Observer-Safe Crisis Assessment And Presentation Contract Decision active.
...
- next run: unversioned `Observer-Safe Crisis Assessment And Presentation Contract Decision`;
...
- the Geography/recognition plan, Activity Resolution reuse audit, Mortal Crisis receipt-contract decision, bounded research, research integration, first lethal-process catalog plan, and care-capability contract are accepted; observer-safe crisis assessment/presentation is active next.
...
3. reassess exact package readiness only after the final consumer is accepted;
