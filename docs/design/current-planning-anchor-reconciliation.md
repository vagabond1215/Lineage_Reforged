...
The roadmap top override and the sequenced-plan current anchor are refreshed through the accepted first lethal-process catalog plan, with the Care Capability, Stabilization, And Process-Effect Contract Decision active.
...
- next run: unversioned `Care Capability, Stabilization, And Process-Effect Contract Decision`;
...
- the Geography/recognition plan, Activity Resolution reuse audit, Mortal Crisis receipt-contract decision, bounded research, research integration, and first lethal-process catalog plan are accepted; the care-capability decision is active next.
...
3. reassess exact package readiness only after both consumer decisions are accepted;
