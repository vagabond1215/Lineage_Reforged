...
The roadmap top override and sequenced-plan current anchor are refreshed through the accepted lethal-process static-foundation version classification, with `Version 0.6.8` active.
...
- the lethal-process static foundation is active next.
...
2. run exact focused/schema/lint checks;
3. run the parent-specific acceptance audit;
