...
- 2026-06-04: Added the pure Runtime Cast Resolver Readiness Helper for `Version 0.5.100`. The helper validates explicit command-like resolver input, target/source descriptors, casting context, runtime policy, optional policy refs, and then delegates to `buildMagicCastReadiness(...)` to return deterministic resolver issues without casting or mutation. Active runtime cast resolver implementation, planned output envelope implementation, command handlers, UI dispatch, effect application, target resolution, resource payment, catalyst consumption/reservation, inventory mutation, combat/Chronicle/quest/account/save/UI event creation, save/account/session schema changes, control/failure/backlash behavior, scroll/tome/document teaching, broader acquisition routes, broader owner scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-06-03: Added the planning-only First Narrow Runtime Cast Resolver Plan for `Version 0.5.99`. The plan defines the future engine-owned resolver request/result boundary, required resolver gates, blocker vocabulary, inert planned output envelopes, and target/effect/resource/catalyst/failure boundaries before implementation. Active runtime cast resolver implementation, command handlers, UI dispatch, effect application, target resolution, resource payment, catalyst consumption/reservation, inventory mutation, combat/Chronicle/quest/account/save/UI event creation, save/account/session schema changes, control/failure/backlash behavior, scroll/tome/document teaching, broader acquisition routes, broader owner scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-06-03: Added the planning-only Magic Command Contract Plan for `Version 0.5.98`. The plan defines future `magic.cast` command/intention shape, explicit known-spell, target, conduit, catalyst, and casting-context descriptors, and a future pure command validation boundary before resolver behavior. Command handlers, runtime cast resolver work, effect application, target resolution, resource payment, catalyst consumption, inventory mutation, combat/Chronicle/quest/account/UI event creation, UI dispatch, save/account/session schema changes, control/failure/backlash behavior, scroll/tome/document teaching, broader acquisition routes, broader owner scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-06-03: Added pure Training Event Acquisition Helpers for `Version 0.5.97`. The helpers validate explicit character-scoped `training_event` acquisition input, return deterministic issues, derive stable known-spell ids when omitted, and propose in-memory `KnownSpellRecordState` records without mutating collections or persisting acquisition events. Active casting, command contracts, runtime cast resolver work, save/account schema changes, persisted acquisition event creation/mutation, scroll/tome/document teaching, teacher/institution/quest/family/Legacy/source-run routes, catalyst consumption, control/failure/backlash behavior, broader ownership scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-06-03: Added the planning-only Known Spell Acquisition Event Plan for `Version 0.5.96`. The plan defines explicit character-scoped `training_event` acquisition ownership and evidence boundaries before mutation, with future helper scope limited to validating training-event input and returning a proposed known-spell record. Acquisition event creation/mutation, active casting, command contracts, UI surfaces, save/account schema changes, scroll/tome/document teaching, teacher/institution/quest/family/Legacy/source-run routes, catalyst consumption, control/failure/backlash behavior, broader ownership scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-06-03: Added the pure Magic Cast Readiness Helper for `Version 0.5.95`. The helper reuses known-spell projection and returns deterministic read-only blockers for known-spell ownership, training-event evidence, conduit metadata, catalyst metadata, control context, spell hook support, spell runtime status, and absent runtime casting. Active casting, acquisition event creation, catalyst consumption, command contracts, control/failure resolution, scroll/tome/document teaching, broader ownership routes, and Magic Legacy access lanes remain deferred.
- 2026-06-02: Added the planning-only Magic Runtime Boundary Plan for `Version 0.5.94`. The plan separates current known-spell ownership/read-only projection helpers from the future pure cast-readiness helper and keeps active casting, acquisition event creation, conduit/catalyst implementation, control/failure resolution, catalyst consumption, runtime cast commands, scroll/tome/document teaching, family/institution/account spell ownership, Magic Legacy power, and the actual `0.5.95` cast-readiness helper deferred until their scoped runs.
- 2026-06-01: Added test-only Magic Runtime Readiness Blocker coverage for `Version 0.5.93`. The tests lock that catalog presence, Arcane Compendium visibility, `PlayerSpellState[]`, broader ownership scopes/routes, missing or unsupported `training_event` evidence, blocked known-spell records, missing conduit/catalyst/control policy, deferred item generation, deferred/unknown hooks, and absent runtime casting all keep spells from being runtime-ready. Active casting, acquisition event creation, conduit/catalyst/control/failure resolution, runtime spell commands, scroll/tome/document teaching, Magic Legacy access lanes, broader ownership/acquisition routes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-06-01: Added pure Known Spell Read-Only Projection for `Version 0.5.92`. The projection summarizes explicit character-scoped known-spell records into available, blocked, and invalid-record counts/lists while reusing collection and training-event evidence validation; catalog presence, Arcane Compendium entries, `PlayerSpellState[]`, account/family/institution/document/item/source-run/heir/Legacy data, lineage, backstory, and UI state still do not imply known spell ownership. Acquisition event creation, active casting, conduit/catalyst/control policy, scroll/tome/document teaching, Magic Legacy access lanes, teacher/quest/discovered-record routes, broader ownership scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-05-31: Added pure Known Spell Acquisition Evidence Helpers for `Version 0.5.91`. The helper boundary now creates, normalizes, validates, and type-checks minimal `training_event` evidence only, while collection validation reuses that evidence validator. Runtime acquisition event creation, active casting, conduit/catalyst/control policy, scroll/tome/document teaching, Magic Legacy access lanes, teacher/quest/discovered-record routes, broader ownership scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-05-30: Added pure collection-level Known Spell Validation Helpers for `Version 0.5.90`. Known-spell collections now validate each character-scoped record, reject duplicate `knownSpellId` values, and require minimal `training_event` evidence without creating acquisition events or enabling broader routes. Runtime acquisition creation, active casting, conduit/catalyst/control policy, scroll/tome/document teaching, Magic Legacy access lanes, teacher/quest/discovered-record routes, broader ownership scopes, and `PlayerSpellState[]` replacement remain deferred.
- 2026-05-28: Updated the planning-only Known Spell Ownership Plan for `Version 0.5.88`. Early known spells should be character-scoped with explicit acquisition evidence; account, family, institution, Legacy, scroll, tome, and document access must not automatically become character spell knowledge. Runtime casting, catalyst behavior, scroll/tome teaching or temporary grants, Magic Legacy power, magic skill gain, family magical tradition inheritance, institution licensing, document-owned study/teaching, generated output, UI, and save/account schema changes remain deferred.
- 2026-05-26: Completed the Combat Equipment Mapping Audit with audit tables and focused mapping tests. Starter `short_bow` combat profiling, starter `butcher_knife` equip/profile decision, hybrid `battle_staff` training, improvised `pickaxe` training, shield/armor/defense skill-gain candidate rules, content-owned equip slots/handedness, broader weapon/armor profile coverage, explicit damage-type handling, and combat formula/balance work remain deferred.
...
- 2026-05-21: Rendered the pure Bloodlines projection in the account meta launcher surface as a read-only Bloodlines section with summary stats, family cards, Family Prestige ledger totals, family unlock summaries, linked run summaries, empty state, and inactive future-system notes. Family management, heirs, heirlooms, bequests, item-instance persistence, estate transfers/claims, Family Prestige earning/spending behavior, Chronicle Marks, Lineage Seals, scoped Backstory Legacy evidence, generated UI output, and broader runtime behavior remain deferred.
- 2026-05-21: Added a pure read-only Bloodlines presentation projection for current account family records, explicit run-history family links, family-scoped unlocks, and Family Prestige ledger totals, with inactive read-only placeholders for heirs, heirlooms, bequests, family management, and Family Prestige spending. React Bloodlines UI, family management, heirs, heirlooms, bequests, item-instance persistence, estate transfers/claims, Family Prestige spending, Chronicle Marks, Lineage Seals, scoped Backstory Legacy evidence, generated UI output, and broader runtime behavior remain deferred.
- 2026-05-21: Added a planning-only Heirloom and Bequest Systems Plan that separates Bloodline upgrades, bequests, heirlooms, estate assets, Family Prestige, Chronicle Marks, Lineage Seals, and Backstory support. Runtime source, schemas, tests, content JSON, UI, generated output, Family Prestige spending, heir systems, item-instance persistence, estate transfers, bequest claims, heirloom registration/transfer, Chronicle Marks, Lineage Seals, and Backstory Eligibility evidence remain deferred.
- 2026-05-21: Cleaned Backstory Legacy creator locked copy, policy metadata, current handoff, and roadmap after the low-risk account-scoped resolver integration. The finished low-risk Backstory Legacy slice now hands off toward Heirloom and Bequest planning; family/source-run/scoped Backstory Legacy evidence, Family Prestige spending, higher-risk candidates, Bloodlines UI, creator purchase UI, generated UI output, and broader creator changes remain deferred.
- 2026-05-21: Wired owned account-scoped Backstory Legacy purchases into creator availability through the runtime ownership helper and resolver evidence path. Family/source-run evidence, Family Prestige spending, scoped storage, creator purchase UI redesign, higher-risk Backstory Legacy candidates, Bloodlines UI, generated UI output, and broader creator changes remain deferred.
- 2026-05-20: Migrated the five low-risk Backstory Legacy purchase candidates into the live Legacy unlock catalog as account-scoped, live, unlock-only ownership/support records: Street Vendor, Net-Tender, Gatherer, Scribe's Apprentice, and Kitchen Hand. Resolver purchase integration, creator availability changes, family/source-run evidence, Family Prestige spending, scoped storage, purchase UI redesign, higher-risk Backstory Legacy candidates, generated UI output, and visible creator availability changes remain deferred.
- 2026-05-20: Added a draft-only, non-runtime Backstory Legacy purchase content catalog for low-risk Tier 1 candidates outside the live Legacy unlock catalog. The draft is not imported by Legacy runtime, account meta purchase presentation, Backstory Eligibility resolver, or character creator availability; live Backstory Legacy purchase records, resolver purchase integration, creator purchase behavior, family/source-run evidence, family prestige spending, region/institution/estate/title scoped storage, purchase UI, generated UI output, and visible availability changes remain deferred.
- 2026-05-19: Added the first runtime-owned family-scoped unlock ownership shape and read-only Backstory Legacy purchase evidence collection helper. Family unlock ownership is stored under current account family state, defaults empty, validates against existing family records and family prestige transactions, and does not grant effects by itself. Backstory Legacy purchase content records, resolver purchase integration, creator integration, region/institution/estate/title scoped storage, Chronicle Marks, Lineage Seals, family prestige earning/spending, purchase UI, heir systems, heirlooms, bequests, generated UI output, and visible backstory availability changes remain deferred.
- 2026-05-19: Added a planning-only Chronicles Bloodline Tree Presentation Plan. Future Bloodlines presentation should live under Chronicles/account meta rather than character creator, derive family trees from current flat family and run-history records, show Family Prestige as ledger-derived family-scoped totals, and keep heirs, bequests, heirlooms, Chronicle Marks, Lineage Seals, family management, Backstory Eligibility family evidence, creator behavior, content JSON, schema changes, and generated UI output deferred.
- 2026-05-19: Added the first runtime-owned family/Bloodline account state shape with empty default family records, optional family linkage fields on run history records, and passive family prestige ledger totals. Family tree UI, Chronicles/Bloodline presentation, heir creation and slots, heirlooms, bequests, Chronicle Marks, Lineage Seals, automatic family prestige earning/spending, Backstory Legacy purchase records, family evidence into Backstory Eligibility, creator behavior changes, visible backstory availability changes, content JSON, and generated UI output remain deferred.
- 2026-05-19: Added a planning-only Legacy Scope and Bloodline Economy Plan and superseded the immediate Backstory Legacy purchase runtime sequence. Family Records and Prestige Ledger Runtime Shape should come next, followed by Chronicles/Bloodline tree presentation planning, then Backstory Legacy purchase runtime/content/resolver work. Family ids on run history, parent/child links, family prestige ledgers, Chronicle Marks, Lineage Seals, heir slots, scoped purchase storage, heirloom item-instance persistence, bequests, estate/title/status ownership, and family-scoped backstory evidence remain deferred; no runtime/source behavior, content JSON, policy metadata JSON, Legacy behavior, creator behavior, visible availability, save/account schema, starter effects, or generated UI output changed.
- 2026-05-19: Added a planning-only Backstory Legacy Purchase Integration Plan now that the creator consumes resolver-backed backstory availability. Future Backstory Legacy purchase runtime shape, purchase content draft, resolver purchase integration, UI/account presentation, family/source-run ledgers, earned skill maxima storage, scoped purchase storage, and blocked evidence owners remain deferred; no runtime/source behavior, content JSON, policy metadata JSON, Legacy behavior, creator behavior, visible availability, save/account schema, starter effects, or generated UI output changed.
- 2026-05-18: Wired the character creator to the runtime-owned Backstory Eligibility Resolver projection for backstory availability. The creator now presents resolver-visible records, disables locked and special records, omits hidden/deferred records from the normal list, and validates selected backstory ids against resolver selectability while keeping settlement-start authorization and snapshot starter effects separate. Backstory Legacy purchase integration, family/source-run evidence ledgers, earned skill maxima storage, blocked evidence owners, broader locked UI polish, generated UI output, and Legacy/economy/magic/status owner systems remain deferred.
- 2026-05-18: Added a planning-only Creator Backstory Presentation Plan for future resolver-backed creator availability display. Creator resolver wiring, locked-backstory UI implementation, Backstory Legacy purchase integration, family/source-run evidence ledgers, earned skill maxima storage, blocked evidence owners, generated UI output, and visible availability changes remain deferred; no runtime/source behavior, content JSON, policy metadata JSON, creator behavior, starter effects, save/account schema, Legacy behavior, UI output, or availability changed.
- 2026-05-18: Added the first runtime-owned Backstory Eligibility Resolver foundation under the game engine: an approved source policy shape, a pure resolver, and focused policy/resolver tests for current live catalog coverage, default safety, blocked evidence, starter-granted skill exclusion, family/source-run scope isolation, no-stacking, non-import boundaries, and no-compatibility guardrails. The resolver is not wired into character creator filtering; creator locked-backstory presentation, Backstory Legacy purchases, family/source-run evidence ledgers, earned skill maxima storage, institution/status/estate/title ownership, magic licensing, mounts, contacts, market/economy effects, medical systems, and oath/paladin behavior remain deferred.
- 2026-05-18: Added a planning-only Backstory Eligibility Resolver Test Plan. Future resolver implementation remains blocked until static policy/content validation, pure resolver, evidence evaluation, scope isolation, default safety, no-stacking, non-import boundary, blocked/special/deferred, creator-boundary, and no-compatibility tests are implemented; no resolver, runtime policy, schema, content JSON, metadata JSON, creator behavior, Legacy behavior, save/account, UI, generated output, or availability changed.
- 2026-05-18: Added the Backstory No-Compatibility guardrail cleanup. Backstory resolver planning now targets current authored content and current account/save shapes only; backwards compatibility, old-save preservation, old-account preservation, migration aliases, retired-id compatibility, converted-id compatibility, historical-id preservation, and migration-only behavior are not planned unless explicitly requested. Next planned step is Version 0.5.55 - Backstory Eligibility Resolver Test Plan.
- 2026-05-18: Added a planning-only Backstory Runtime Policy Shape Draft for future resolver work. The draft defines a future approved policy container, availability statuses, rule and requirement shapes, scope and source-attribution rules, missing/blocked-data behavior, no-stacking policy, default safety, example rules, validation expectations, and current-data strategy; no runtime policy file, schema, resolver, content JSON, metadata JSON, creator filtering, Legacy purchase, save/account, UI, generated output, or availability behavior changed.
- 2026-05-17: Added a planning-only Backstory Evidence Ownership design for future resolver work. Evidence remains blocked from runtime until source attribution, earned-skill maxima, family/source-run ledgers, scoped reputation/renown, title/estate/institution/contact ownership, current-data behavior, and runtime policy tests exist; no evidence storage, resolver, runtime policy data, content JSON, metadata JSON, Legacy behavior, schema, creator UI, or availability behavior changed.
- 2026-05-17: Added a planning-only Backstory Eligibility Resolver design before runtime implementation. Future filtering remains blocked until evidence ownership, runtime policy shape, current-data behavior, family/source-run ledgers, earned-skill maxima, and creator explainability are designed and tested; no resolver, policy metadata import, live availability change, content JSON change, Legacy purchase, schema change, or UI change was added.
- 2026-05-17: Added Drover's Hand and Kitchen Hand as the Tier 1 backstory follow-up from the approved first-batch plan. The records use current canonical skill ids, add no starting abilities, and do not add unlock/resolver logic, runtime policy imports, mounts/riding/cavalry behavior, pack-animal effects, contacts, inn ownership, passive economy behavior, free food generation, magic access, or generated UI output. Dockhand/Riverhand, Barge Hand, Forge Yard/Forge Hand, Forge Apprentice, Stablehand, Tanner's Yard, Loomhouse, Hidden Blood, Unacknowledged Blood, Red-Lantern Ward, and Courtesan's House remain deferred or later planning concepts.
- 2026-05-17: Added the first five safe Tier 1 backstory records: Militia Levy, Street Vendor, Net-Tender, Gatherer, and Scribe's Apprentice. The records use current canonical skill ids, add no starting abilities, and do not add unlock/resolver logic, runtime policy imports, market passives, contacts, mounts, heir/status ownership, magic access, or economy behavior. Drover's Hand and Kitchen Hand remain planned for a later content batch.
- 2026-05-17: Planned the first safe Tier 1 backstory content batch without adding live records. Recommended later implementation candidates are Militia Levy, Street Vendor, Net-Tender, Gatherer, Scribe's Apprentice, Drover's Hand, and Kitchen Hand, all using current skill ids and explicitly excluding runtime unlock logic, abilities, market passives, contacts, mounts, heir/status ownership, magic access, and economy behavior until later owned systems exist.
- 2026-05-17: Added planning-only tier/lane/branch, precursor, alternate-unlock, cap, extra-effect, and upgrade-scale metadata to the non-runtime Backstory Legacy policy draft, plus compact future lane drafts for militia, trade, craft, river/coastal, medicine, scholar, oath, lineage, and social-patronage concepts. These drafts are not live backstory records and remain blocked from character creator, Legacy runtime, starter skills, save/account storage, combat, magic, economy, progression, and UI output until a reviewed resolver and owning runtime systems exist.
- 2026-05-15: Revised the planning-only Backstory Legacy metadata model before implementation. Future drafts should use `tier_1`, `tier_2`, `tier_3`, `special`, and `deferred` rather than a `standalone` tier or branch-role category; missing precursors should be represented through explicit precursor and alternate-unlock fields. Long-term Backstory Legacy upgrade scale remains planning-only and may support large incremental ranges if prestige, Echo, and cost curves justify them, but no runtime metadata, creator filtering, Legacy purchasing, save/account, or starter-skill behavior changed.
- 2026-05-13: Completed the small app/UI combat skill-gain notification consumer by projecting transient combat `skillGainMessages` from consumed `GameDelta[]` into top-bar notification items through the existing presenter. This remains UI/app-layer only: structured Chronicle entries, durable combat history/report ownership, account/profile writes, and broader encounter report semantics remain deferred.
- 2026-05-13: Started the first safe read-only magic UI slice by adding an Arcane Compendium section to the in-game Codex. The section projects all 55 authored spells as static compatibility references with profile tags, catalyst-family metadata, hook summaries, and warnings for partial/deferred/runtime-blocked records. This does not create player spell ownership or casting: spell acquisition, castability, catalyst execution, runtime magic behavior, magic skill gain, Magic Legacy, and save/account schema changes remain deferred.
- 2026-05-13: Added Beta-batch compatibility profiles to eight existing partial spells and promoted those hook-safe records to `ready`, while profiling Drain but keeping it `partial` until lifesteal coupling is runtime-owned. The live spell catalog now has 23 ready, 5 partial, 27 deferred, and 28 profiled spells. Curse, Berry, Bloom, War Song, and Drain remain partial because deferred debuff, generated-item, regeneration, performance-song, and drain semantics are not implemented. This remains metadata/validation/test-only: new spells, runtime magic expansion, combat behavior, acquisition, catalyst effects, affinity/resistance behavior, magic skill gain, Legacy, account, preparation, payout, and UI work remain deferred.
- 2026-05-12: Added Alpha-batch compatibility profiles to 12 existing partial spells only and promoted those hook-safe records to `ready`, moving the live spell catalog to 15 ready, 13 partial, 27 deferred, and 19 profiled spells. This remains metadata/validation/test-only: new spells, runtime magic expansion, combat behavior, acquisition, catalyst effects, affinity/resistance behavior, magic skill gain, Legacy, account, preparation, payout, and UI work remain deferred.
- 2026-05-10: Clarified the magic/spellbook handoff before pausing heavier work: `docs/design/magic-system-charter.md` and `docs/design/spellbook-expansion-blueprint.md` remain design-only, all 55 current spells now carry top-level `primaryFamily`, and Step 50 should be the next metadata-only implementation slice for existing partial spell compatibility profiles. Do not add new spells, runtime magic, UI, acquisition, catalyst effects, affinity/resistance behavior, magic skill gain, or magic Legacy work before that slice is intentionally resumed.
- 2026-05-10: Added top-level `primaryFamily` spell metadata plus validation/type/test coverage for all 55 authored spells, using the spellbook expansion blueprint's current-catalog remap as the source of truth. This remains metadata-only: compatibility profiles, spell additions, runtime magic behavior, catalyst effects, affinities, acquisition, magic skill gain, Legacy, account, preparation, payout, and UI work remain deferred.
- 2026-05-10: Added `docs/design/spellbook-expansion-blueprint.md` as a design-only adaptation of the approved spellbook expansion research. The blueprint preserves the classless conduit/catalyst taxonomy, maps the current 55 spells into one-primary-family classifications, records rename/reclassification recommendations, captures the 240-320 long-term catalog target with a 256-concept reference appendix, and keeps runtime spell expansion, `spells.json` edits, schema changes, acquisition, catalyst effects, affinities, magic skill gain, Legacy, account, preparation, payout, and UI work deferred.
- 2026-05-09: Added spell-level compatibility status guardrails for all authored spells so future metadata expansion and read-only UI can distinguish ready, partial, deferred, and placeholder records without inferring readiness from `compatibilityProfile` presence alone. Read-only compatibility UI remains blocked until enough spells are `ready` with complete profiles; runtime spell expansion, catalyst effects, magic skill gain, and magic Legacy behavior remain deferred.
- 2026-05-09: Added stricter validation-only guardrails for magic conduit and catalyst metadata. Melee weapons remain valid conduits when their authored tag profile is form-appropriate, while overbroad staff-like dagger/sword/projectile profiles, catalyst family drift, and catalyst profiles on non-material records now stay blocked. Material/channeling efficiency, broader spell/item metadata coverage, runtime spell execution, catalyst effects, magic skill gain, and magic Legacy behavior remain deferred.
- 2026-05-09: Added Phase 2 validation-only magic metadata support for conduit/casting tag vocabulary, spell compatibility profiles, and catalyst tier profiles on a small representative content subset. Runtime spell execution expansion, catalyst effects, affinity/resistance behavior, scroll/tome behavior, magic skill gain, and magic Legacy power remain blocked behind later phased implementation slices.
- 2026-05-09: Added `docs/design/magic-system-charter.md` as the design-only boundary for future magic runtime expansion, defining classless known-spell + conduit-tag + optional-catalyst + control-capacity casting, explicit freecasting penalties, and phased implementation gates. Runtime spell execution breadth, scroll/tome behavior, magic skill gain, affinity/resistance systems, and direct magic Legacy power remain deferred until phased follow-up slices are implemented with validation guardrails.
...
- Intended implementation:
  - add current-content combat mapping for starter `item.short_bow` before any combat formula rewrite so hunter starts have a real ranged action and archery skill mapping
  - decide whether starter `item.butcher_knife` should stop auto-equipping as a weapon or receive a dagger/knife combat profile
  - decide whether hybrid starter `item.battle_staff` and improvised `item.pickaxe` should train through the current combat skill-gain candidate rules or remain explicitly non-training
  - replace the UI-authored starter equipment substring heuristic with content-owned equip slot, offhand/shield, and handedness metadata before broad equipment behavior expands
