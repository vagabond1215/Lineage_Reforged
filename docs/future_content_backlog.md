...
- Prerequisite: semantic validation, evidence contract/schema/validator work, progress-state planning, and progress-schema planning have landed; next implement only the strict progress record schema, then plan and implement progress semantic validation before evidence-to-progress rules, producers, completion behavior, and later trial/checkpoint integration
