...
- Intended implementation:
  ...
  - the UI character creator now runs as a deterministic multi-step flow for lineage, identity, continent, region, settlement, backstory, path, and review, with lineage-valid identity palettes, settlement-aware backstory pools, a narrative live summary, and a snapshot factory kept separate from the screen component
