...
- 2026-05-18: Added a planning-only Backstory Runtime Policy Shape Draft for future resolver work. The draft defines a future approved policy container, availability statuses, rule and requirement shapes, scope and source-attribution rules, missing/blocked-data behavior, no-stacking policy, default safety, example rules, validation expectations, and migration strategy; no runtime policy file, schema, resolver, content JSON, metadata JSON, creator filtering, Legacy purchase, save/account, UI, generated output, or availability behavior changed.
- 2026-05-17: Added a planning-only Backstory Evidence Ownership design for future resolver work. Evidence remains blocked from runtime until source attribution, earned-skill maxima, family/source-run ledgers, scoped reputation/renown, title/estate/institution/contact ownership, migration behavior, and runtime policy tests exist; no evidence storage, resolver, runtime policy data, content JSON, metadata JSON, Legacy behavior, schema, creator UI, or availability behavior changed.
- 2026-05-17: Added a planning-only Backstory Eligibility Resolver design before runtime implementation. Future filtering remains blocked until evidence ownership, runtime policy shape, migration behavior, family/source-run ledgers, earned-skill maxima, and creator explainability are designed and tested; no resolver, policy metadata import, live availability change, content JSON change, Legacy purchase, schema change, or UI change was added.
