...
- Intended implementation:
  ...
  - the character creator now serves lineage and continent card art from `apps/rpg-ui/public/character-creator`, brings decorative art to the foreground on hover for image-backed cards, and defaults the live summary to collapsed on continent, region, and settlement steps so those cards can claim more space
